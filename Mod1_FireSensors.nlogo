...
to setup
  ...
  let trees-counter 0
  set trigger-count 0
  set total-trees int ( 250 * 250 * density / 100 )
  set trees-per-sensor int ( total-trees / total-number-of-sensors )

  ask patches [
    if (random 100) < density
    [
      set trees-counter trees-counter + 1
      (ifelse
        trees-counter mod trees-per-sensor = 0
        [set pcolor white]
        [set pcolor green ])
  ]
  ]
  let sparks 0
  while[sparks < max-sparks][
  ask one-of patches [ set pcolor red ]
  set sparks sparks + 1
  ]


  set initial-trees1 count patches with [pcolor = green]
  set initial-trees2 count patches with [pcolor = white]
  set initial-trees initial-trees1 + initial-trees2
  reset-ticks
end
...
to go
  ...
  ask patches with [ pcolor = red ] [
    ask neighbors4 with [ pcolor = green or pcolor = white ] [
      let x pcolor
      if random 100 < probability-of-spread [
      set pcolor red
      if x = white  and random 100 < confidence-interval [
          set trigger-count trigger-count + 1
          if trigger-count mod effective-trigger = 0
          [set probability-of-spread probability-of-spread - fire-fighting-effectiveness]
        ]
      ]
    ]
    set pcolor red - 3.5
